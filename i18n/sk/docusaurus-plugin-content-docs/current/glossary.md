...
**Chunk overlap (prekryv)** — spoločný úsek textu u susedných chunkov. Zachráni fakt, ktorý padol na líniu
...
blízkosť susedných viet (zmena témy). Drahšie, ale každý chunk potom vyjde „o jednej veci“.
...
naložiť hranice chunkov a spriemerovať vektory tokenov v každom z nich — tak vektor chunku nesie kontext
...
výraznejšia, ale drahšia na pamäť, rýchlosť vyhľadávania aj peniaze.
...
kontrastným učením na trojiciach (dopyt, správny úryvok, nesprávny úryvok), poháňaným tvrdými negatívmi (hard
negatives). Vynúti si úplnú preindexáciu korpusu, keďže dopyt aj dokument musia zdieľať jednu verziu modelu.
...
(frekvencie termov). Zachytáva presné tokeny, slepé na synonymá. ↗ [Wikipedia](https://en.wikipedia.org/wiki/Okapi_BM25)
...
skóre, ale je krehké voči odľahlým hodnotám a posunom rozdelenia medzi dopytmi — je to na skóre založená
alternatíva k RRF založenému na poradí.
...
alebo listwise. Zero-shot a riaditeľný inštrukciou, ale drahý, latenčný a nedeterministický — na rozdiel od
...
no výberový filter sa môže dostať do sporu s ANN-indexom) alebo po ňom (rýchle, ale výberový filter môže
...
**Lost-in-the-middle (strata uprostred)** — model využíva informáciu pochovanú v strede dlhého kontextu horšie
...
**Self-consistency (sebakonzistencia)** — namiesto jediného greedy dekódovania reťazca úvah (chain-of-thought)
...
**Answer relevance (relevancia odpovede)** — reference-free generation metrika (bez etalónu): odpovedá odpoveď
...
**Faithfulness (opretosť o kontext)** — reference-free generation metrika (bez etalónu): odpoveď rozložíš na
...
**Least privilege / tool allow-listing (princíp najmenších oprávnení)** — obmedzenie množiny nástrojov
...
**Feedback loop (observability → eval)** — produkčné zlyhania a spätná väzba od používateľov sa stávajú novými
...
podielu z identifikátora trace; lacné a bezstavové, ale slepé k tomu, ako sa požiadavka skončila, takže nevie
prednostne ponechať zlyhania.
...
dashboard) a požiadavku pustí; tvrdý strop vynucuje za behu (odmietne, spustí na lacnejší model, oreže
kontext).
...
**Routing / query router (smerovanie dopytov)** — najľahšia úroveň agentnosti: model urobí jediné rozhodnutie — kam poslať dopyt (do ktorého indexu/nástroja, alebo „vyhľadávanie netreba") — a ďalej je tok statický. Nezamieňať s model routing — voľbou, ktorý model odpovedá (Časť III).
...
**Corrective RAG, CRAG (korektívny RAG)** — nadstavba nad ľubovoľným RAG: ľahký hodnotiteľ vyhľadávania (retrieval evaluator) ohodnotí nájdené dokumenty a zaradí svoje skóre dôvery do troch priehradok: Correct → spresniť (ponechať iba relevantné fragmenty), Incorrect → zahodiť a prejsť na webové vyhľadávanie, Ambiguous → skombinovať oboje. Funguje plug-and-play nad hociktorým RAG. ↗ [arXiv](https://arxiv.org/abs/2401.15884)
...
**Parallel tool calls (paralelné volania nástrojov)** — niekoľko nezávislých volaní nástrojov, ktoré model vydá v jednom ťahu; behové prostredie ich rozdelí (fan-out — spustí naraz) a pozbiera výsledky (fan-in). Sú platné iba vtedy, keď volania na sebe nezávisia a navzájom si neprekážajú; riadi sa to poflagovo podľa poskytovateľa (`disable_parallel_tool_use`, `parallel_tool_calls`).
...
**Non-termination (nezastavenie cyklu)** — príznačné zlyhanie agentnej slučky: nikdy sa nezastaví, zasekne sa na opakovanej akcii alebo sa odkloní od cieľa.
...
**Orchestrator / supervisor (orchestrátor / supervízor)** — vedúci agent, ktorý rozloží úlohu, rozdelí
podúlohy medzi agentov-vykonávateľov a poskladá ich výsledky; jeho „nástrojmi" sú samotné subagenty.

**Worker / sub-agent (agent-vykonávateľ / subagent)** — špecializovaný agent, ktorý spracuje pridelenú
podúlohu a vráti výsledok.

**Handoff (odovzdanie riadenia)** — odovzdanie riadenia spolu s relevantným kontextom od jedného agenta
druhému; správa o odovzdaní pôsobí ako prompt pre prijímajúceho agenta.

**Agent chain (reťaz agentov)** — sekvenčná topológia, v ktorej každý agent pretvára výstup predchádzajúceho
...
v pár kolách kritizuje a upravuje cudzie návrhy a zbieha sa k presnejšiemu a konzistentnejšiemu výsledku než
...
rozsah podľa najmenších oprávnení určený klientom, nie ponechaný na dohodu.
...
**Inference (inferencia)** — model počíta výstupy zo vstupov v produkcii — dopredný prechod ako služba, na
...
batching, správa KV-cache, OpenAI-kompatibilné API (vLLM, SGLang, Ollama).
...
batchu a opúšťajú ho na granularite tokenu namiesto čakania na dokončenie celého batchu; hlavná páka
priepustnosti.
...
scale-to-zero platí cenu pri ďalšej požiadavke.
...
Súbežnosť pochádza zo slučky udalostí, nie z počtu workerov; workery pridávajú jadrá a pokrývajú CPU-viazané
útržky (serializácia, tokenizácia, JSON).
...
**Backpressure (protitlak)** — zámerné ohraničenie práce v behu: semafor obmedzuje počet súbežných
...
**Load shedding (odhadzovanie záťaže)** — rýchle odmietanie nadbytočných požiadaviek, keď je fronta plná
...
**Little's Law (Littlov zákon)** — identita z teórie frontov L = λW: priemerná súbežnosť sa rovná miere
...
**Quantisation (kvantizácia)** — ukladanie váh (a voliteľne aktivácií) v nižšej presnosti — FP8, INT8 alebo
...
**KV-cache quantisation (kvantizácia KV-cache)** — ukladanie samotnej KV-cache v nižšej presnosti (napríklad
...
zmierňujú snímky pamäte a teplé fondy.
...
**Data residency (rezidencia dát)** — záruka o tom, kde sa požiadavky spracúvajú (región alebo geografia);
...
suverenita. Odlišná od rezidencie, ktorá je len o tom, kde dáta ležia.
