...
# Реальные агенты — три вывески, одни и те же приёмы

Часть II разобрала агента по деталям: цикл, руки-инструменты, план, восстановление после сбоя,
ограничители, команды и, наконец, разъём, которым всё это подключается к внешнему миру. Каждый приём мы
брали абстрактно — как устроен, где ломается, когда лучше не применять. Пора свести абстракцию с
реальностью и посмотреть, во что эти приёмы превращаются у трёх агентов, с которыми инженер имеет дело
первым делом: Claude, OpenAI, Gemini.

Урок финальный и нарочно не вводит новой теории. Он прогоняет каждую тему Части II по всем трём поставщикам
и показывает главное: за дюжиной несовместимых API прячется один и тот же приём. Меняются имена полей и
форма, в которой всё это едет по проводу; идея — та же. Отсюда правило, которым связан весь урок. В каждом разделе мы сначала называем
**долговечный приём** — то, что переживёт любую смену версий, — потом смотрим, как его реализует каждый из
троих *сегодня*, с датой в руках; разбираем инженерную разницу; и находим, где всё это ломается, — с
...
сдвинуться — API поставщиков живут быстро. Поэтому запоминай приём, а деталь поставщика держи как то, что
...
IBM формулирует ровно то напряжение, на котором стоит весь этот урок: у настоящих агентов агентность стоит
латентности и сложности, поэтому инженерия — это выбрать *наименьшую* свободу, которая решает задачу, а не
наибольшую.
...
[использование инструментов](./tool-use.md), и снаружи он тождествен у всех троих. Различается только форма
обмена.
...
- **Claude** объявляет инструменты массивом `tools` (`name`, `description`, `input_schema` = JSON Schema).
  Обмен едет блоками контента: модель возвращает `stop_reason: "tool_use"` с блоками `tool_use`, а ты
...
аргументы, выдумка поверх результата. Лечение везде общее — строгие схемы и узкий, непересекающийся набор
инструментов. Выбор поставщика не освобождает тебя от [проектирования инструментов](./tool-use.md): плохое
...
[Agentic RAG](./agentic-rag.md) — «поиск становится действием» — только теперь встроенный в платформу.
...
- **OpenAI** даёт хостируемый веб-поиск `{type:"web_search"}` с инлайновыми аннотациями `url_citation` и
  ...
  семантический RAG как управляемую встройку, возвращающую аннотации `file_citation`.
...
Engine. Урок про провалы из Части I тут не отменяется: встройка не чинит **retrieval-провал (retrieval
failure)**, она лишь переносит вопрос, кто крутит поиск.
...
retrieval-провал против generation-провала, что разбирался в Части I.
...
шагов страхует от того, что цикл не остановится. И то, и другое от поставщика не зависит — как и учил урок про
...
- **Claude** крутит цикл, пока не придёт `stop_reason:"end_turn"`; в Claude Agent SDK функция `query()`
  ...
  `max_turns` и `max_budget_usd`. Рассуждение видимо — оно едет блоками `thinking`; **чередующееся
...
- **OpenAI** отмеряет рассуждение параметром **уровень рассуждения (reasoning effort)** — `reasoning.effort`
  ...
  внутренние и непрозрачные, ты за них платишь как за выход, но не видишь. Цикл инструментов гоняет
...
- **Gemini** даёт **бюджет размышления (thinking budget)** — числовой регулятор на запрос (`thinkingBudget`;
  ...
  выпускает `Event` и на этом выпуске встаёт на паузу, пока Runner не зафиксирует состояние.
...
Ломается это предсказуемо — незавершением цикла: цикл, который не встаёт, или **уход от цели**, когда агент
...
прямая дорога к истории с поля про то, как судить о прогрессе по настоящему состоянию, а не по отметке
времени.
...
пост-проверка осматривает выход; слой политики решает, что вообще разрешено. Это принцип наименьших
привилегий из урока про [использование инструментов](./tool-use.md), превращённый в машинерию.
...
- **OpenAI** даёт в Agents SDK входные и выходные ограничители (`@input_guardrail`/`@output_guardrail`),
  которые при срабатывании поднимают исключение-растяжку (tripwire): входные бегут параллельно агенту,
  выходные — после его завершения. Плюс хуки жизненного цикла (`RunHooks`/`AgentHooks`) и одобрение человека
  на уровне инструмента (`needs_approval` → прогон встаёт, продолжаешь approve/reject). Отдельно —
  бесплатный Moderation API (`omni-moderation-latest`, 13 категорий).
- **Gemini/ADK** дают **колбэки (callbacks)** — фиксированную матрицу из шести точек: `before/after_agent`,
  `before/after_model`, `before/after_tool`, — где возвращённый объект закорачивает вызов (например,
  `before_tool`, вернувший словарь, пропускает исполнение). Плюс внутримодельные настройки безопасности
  (четыре категории вреда с порогами; по умолчанию Off на Gemini 2.5/3 — по докам от 1 июня 2026) и Model Armor — отдельно
  разворачиваемый сервис, просеивающий промпты и ответы на инъекции, PII и вредоносные URL (обновление от
  10 июля 2026).
...
внутримодельную безопасность плюс привинчиваемый сервис Model Armor. Идея та же — расставь проверки,
дай наименьшие привилегии, — а мест, куда их приткнуть, три.

Рвётся всё на простом: ограничитель, который можно обойти, — не ограничитель. Опасна именно пишущая,
действующая функция, до которой дотянулись через prompt injection (внедрение инструкций в текст) — риск из
урока про [использование инструментов](./tool-use.md).
...
([мультиагентные системы](./multi-agent.md)).

:::tip[▶ Видео]

<YouTube id="ZVPlLaehjLk" title="Agentic AI Frameworks Explained: Workflows, Multi-Agent, & Production — IBM Technology" />

IBM проходит ровно тот скачок, который этот раздел делает предметным: от одиночного цикла к мультиагентным
рабочим потокам в проде.
...
фреймворк, а не модель. Это ровно мысль урока про [фреймворки оркестрации](./orchestration-frameworks.md):
...
исполнители, делящие общую изменяемую рабочую область, сталкиваются. Отсюда — история с поля про изоляцию
через рабочие деревья.
...
инструменту: `M × N` попарных коннекторов схлопываются в `N + M` ([MCP](./mcp.md)).
...
API-коннектор — только удалённый, тогда как локальный stdio закрывает Claude Code. OpenAI — со-учредитель и
...
| Мультиагентность | изолированные субагенты (возвращается финальное сообщение) | передача управления против агента-как-инструмента | координатор + детерминированные workflow-агенты |
...
## Не абстракция: четыре истории с поля
...
- **Судить о прогрессе по настоящему состоянию, а не по отметке времени (→ §4/§5).** На вопрос «оно
  завершилось?» отвечают, сверяясь с эталоном истины, — по слитому состоянию PR, по статусу деплоя, но никогда по
...
становится справочником, а не переучиванием с нуля. А когда API сдвинутся (они сдвинутся), приём — это то,
что у тебя останется. В этом и защита от **привязки к поставщику (vendor lock-in)**.

## Что забрать из урока

- Руки любой агент получает одинаково: объявляешь инструмент (имя + описание + JSON Schema), модель порождает
  структурированное намерение, твой код исполняет его, результат едет обратно. Различается только форма
...
- Восстанавливайся, а не умирай: ошибки инструмента едут обратно текстом для самокоррекции; сохраняй
...
- Дели на оркестратор и изолированных исполнителей только тогда, когда один цикл не справляется: субагенты у
  Claude, передача управления против агента-как-инструмента у OpenAI, координатор с детерминированными
...
- MCP связывает всё воедино — написан Anthropic (2024), передан Agentic AI Foundation под Linux Foundation
  (декабрь 2025); все трое сегодня MCP-клиенты. Учи долговечный приём, а поставщик — это датированный случай.
