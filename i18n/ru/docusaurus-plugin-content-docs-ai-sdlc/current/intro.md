...
1. **[Узкое место проверки](./part-1-foundation/verification-bottleneck.md)** — тезис курса, выведенный из
   доказательств.
2. **[Как читать доказательства](./part-1-foundation/reading-the-evidence.md)** — та самая лестница `MEASURED` / `REPORTED` / `ASSERTED` в работе.
3. **[Подготовка важнее модели](./part-1-foundation/preparation-over-model.md)** — почему то, что подаёшь агенту, весит больше, чем выбор самой модели.
4. **[Память проекта и разбиение по уровням](./part-1-foundation/project-memory-and-tiering.md)** — долгая контекстная опора, с которой работает флотилия.
5. **[Правила, которые держатся](./part-1-foundation/rules-that-hold.md)** — как превратить договорённости в ограничения, соблюдение которых обеспечивает машина.
