...
Три вещи проходят через все стадии пайплайна и не сводятся к одному шагу: как узнать, что система работает
(**Evaluation**), как удержать её безопасной (**Guardrails**) и как видеть, что она делает в проде
(**Observability**). Начинаем с оценки — без неё остальное настраивать нечем.

## Evaluation (оценка)

Мы уже четырежды упёрлись в «это измеряют метриками» — на чанкинге, retrieval, реранкинге и faithfulness.
...
демо, и про это чаще всего копают на интервью.

### Главный принцип: retrieval и generation оценивают отдельно

Декомпозиция ошибки из первого урока здесь становится рабочим инструментом. Две стадии ломаются по-разному и
...
**golden set**. Берут его руками или синтетически — LLM генерирует пары «вопрос-ответ» по корпусу, а человек
вычитывает. Маленький, но чистый набор лучше большого и шумного. Именно здесь команды недоинвестируют, а без
эталона весь eval рушится.
...
берут **другую LLM в роли судьи**: она оценивает ответ по рубрике или сравнивает с эталоном и выставляет
балл. Приём масштабирует человекоподобную оценку на тысячи примеров — ключевая техника generation-eval и
...
У судьи есть смещения, и о них надо знать: **position bias** (предпочитает первый из двух ответов),
**verbosity bias** (длиннее — значит лучше), **self-preference** (свой стиль оценивает выше). Отсюда
правила: давать чёткую рубрику и **валидировать судью против человеческих оценок**, прежде чем ему доверять.

:::tip[▶ Видео]

<YouTube id="dAE7OFm9oek" title="Can You Trust an AI to Judge Fairly? Exploring LLM Biases — IBM Technology" />

Смещения судьи и можно ли ему доверять.
...
- **Online:** мерить в проде — фидбэк пользователей (палец вверх/вниз), неявные сигналы, A/B-тесты. Реальные
  запросы показывают то, что golden set упустил.
...
- **LLM-as-judge** оценивает свободный текст по рубрике; помни про смещения, валидируй против людей.
- **Offline** (регрессии в CI) + **online** (фидбэк, A/B).
- Метрики говорят, какую стадию чинить.

**Новые термины** → [Глоссарий](../glossary.md): evaluation, golden dataset / ground truth, answer
...
🚧 **Guardrails** (prompt injection, PII, безопасность на входе и выходе) и **Observability** (трейсинг,
логирование чанков, стоимость и задержки).
