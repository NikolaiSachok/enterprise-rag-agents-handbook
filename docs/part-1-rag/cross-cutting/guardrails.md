...
- **Валидация выхода.** Проверять ответ перед выдачей: не утекли ли секреты, нет ли PII (персональных данных), не нарушена ли политика.
...
Отказывать во вредных или недопустимых запросах и фильтровать токсичный или не соответствующий политике вывод. Всегда **две поверхности**: скан входа (блокировать вредное/не по теме) и выхода (блокировать небезопасный ответ). А в RAG добавляется третья точка — **ingestion**: отравленный документ лучше ловить при индексации, а не только на запросе. (Инструменты — Guardrails AI, NeMo Guardrails, Llama Guard, Granite Guardian — отдельный слой об инструментах, о нём позже; здесь мы про принцип.)

## Guardrails — не серебряная пуля

Полной защиты не бывает: это **defense-in-depth** (эшелонированная защита), слои. И есть компромисс с удобством: слишком строго — отказываешь в легитимных запросах. Поэтому guardrails тоже **измеряют**: долей успешных атак (attack success rate, ASR) на наборе атак — это снова мостик к Evaluation.
...
**Новые термины** → [Глоссарий](../../glossary.md): guardrails, prompt injection (direct / indirect), spotlighting, instruction hierarchy, PII redaction, input/output validation, content safety / moderation, jailbreak, least privilege / tool allow-listing, attack success rate, defense-in-depth.
