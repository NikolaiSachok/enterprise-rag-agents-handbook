...
**[New terms](../../../glossary.md)**: context precision, context recall, faithfulness (formula), answer relevance (formula), reference-free vs reference-based evaluation, LLM-judge calibration, position bias, verbosity bias, self-preference / self-enhancement bias, pointwise vs pairwise evaluation, inter-annotator agreement (Cohen's kappa, Fleiss' kappa), active sampling / active learning.
