...
шаг — **span**. Трейс отвечает на главный вопрос отладки: «почему на этот запрос вышел именно такой ответ?»
...
## Стоимость и латентность — граждане первого класса
...
Прод-трейсы и фидбэк пользователей становятся новыми eval-кейсами — теми самыми трудными и провальными
...
*(Инструменты — LangSmith, LangFuse, Arize Phoenix, OpenTelemetry — это слой Части III; здесь мы про
принцип.)*
...
- **Стоимость и латентность** — первого класса (токены = деньги, вызовы медленные).
- Observability **кормит eval**: прод-провалы → новые кейсы golden set → eval-driven development.

**Новые термины** → [Глоссарий](../../glossary): observability, trace / span, RAG tracing, cost per
