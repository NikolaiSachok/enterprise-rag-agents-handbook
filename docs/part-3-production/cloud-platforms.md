...
(inference server) — можно держать самому, а можно арендовать, и тогда на его месте остаётся чужой
...
контракт, чьи роли доступа действуют и в какой счёт падают токены.

Всего способов потреблять модель три, и они выстраиваются вдоль одной оси «контроль — удобство».
...
поданные как **управляемые эндпоинты (managed endpoints)** внутри облака, где уже живут твои данные и
сервисы.
...
Что такое инференс — то единственное, что все эти платформы на самом деле продают, — и почему гонять его
под нагрузкой — отдельная дисциплина.
...
API Anthropic. За что тогда платить платформе?» За периметр. Продукт здесь — не модель, а всё, что облако
оборачивает вокруг неё:

- **Единая аутентификация.** Доступ к модели раздают те же IAM-роли, что и к остальной инфраструктуре, —
  без второго набора API-ключей, живущего своей жизнью.
- **Единый биллинг.** Токены падают в тот же облачный счёт, что виртуалки и базы, и на них
...
- **Соответствие в наследство.** Сертификации облака распространяются на платформу: SOC 2,
  HIPAA-eligible сервисы, инструменты для соблюдения GDPR.
- **Журналы аудита и квоты.** На проект и команду: видно, кто сколько сжёг, и задано, кому сколько можно.
...
Игроков три — по числу больших облаков, и у каждого своя история имени.
...
Модели в ней подаются как Foundry Models, с внутренним делением на «Models sold by Azure» (модели, которые
...
(сокращённо GEAP); к маю переехала консоль, API-эндпоинты остались прежними, а документация пока честно живёт
посередине переезда — часть страниц под старым именем, часть под новым.

Заметь, что произошло: две платформы из трёх сменили имя примерно за год. Это нормальный темп индустрии,
и он подсказывает, как учить материал. Имена продуктов и состав бандлов тасуются постоянно; переживают
эту чехарду категории возможностей — каталог моделей, гарантии приватности и резидентности, управляемый
RAG, платформенные ограничители (guardrails), модель пропускной способности и оплаты. Выучи категории — и
любое свежее имя само разложится по полкам, а любое название из этого урока считай снимком на дату. В
[уроке про MCP](../part-2-agents/mcp.md) мы уже договаривались держаться за разделительные черты, потому
что списки имён устаревают быстрее, чем печатается страница. Здесь тот же приём.

## Каталоги моделей — чьи модели кто подаёт
...
Bedrock строился мультивендорным с первого дня: Anthropic (Claude), Meta (Llama), Mistral, Cohere,
...
Foundry. Связка «модель → облако» слабеет, и центр тяжести сравнения смещается в обвязку: резидентность,
уровень управляемого RAG, экономика выделенной ёмкости. К ним и переходим.
...
Мелкий шрифт при этом различается: Google формулирует «по умолчанию», а у Azure есть оговорка про
мониторинг злоупотреблений — содержимое, зацепившее фильтры, может просмотреть живой человек, если этот
...
с запозданием. Поэтому у каждой платформы есть свой регулятор «резидентность или ёмкость», под своими
именами: у Azure — типы развёртывания (Standard regional, Data Zone, Global), у Bedrock — межрегиональный
инференс с географическими профилями, ограниченными US / EU / APAC, либо глобальными, у Vertex/GEAP —
региональные эндпоинты либо глобальный (он даёт больше ёмкости — и никаких гарантий
резидентности).
...
соответствия, после которой юридический отдел говорит системе «да». На практике именно эта триада чаще
...
Поверх голых эндпоинтов платформы продают «батарейки» — готовые уровни, собранные из знакомых тебе
деталей.

Первая батарейка — **управляемый RAG (managed RAG)**: весь конвейер Части I — загрузка документов,
...
У Azure поисковой основой служит Azure AI Search, а упакованный уровень опоры ответов на данные сейчас
называется Foundry IQ; его предшественник On Your Data доживает последние месяцы — до октября 2026-го.
У Google — RAG Engine плюс корпоративный поиск Vertex AI Search, который прямо сейчас перевешивают под
...
Отдаёшь ручки, которым была посвящена половина Части I, — стратегию чанкинга, веса гибридного поиска,
выбор реранкера, точки для подключения оценки; что из этого доступно, зависит от продукта, а часть
настроек просто зашита. Отсюда трезвое правило: управляемый уровень — разумное умолчание для стандартного
корпуса, собственный конвейер — когда [оценка](../part-1-rag/cross-cutting/evaluation.md) показывает, что
умолчания не дотягивают. Команды, которым нужно всерьёз крутить качество по eval-циклу, из управляемого
RAG нередко вырастают — или оставляют ему только загрузку и хранение.

Вторая батарейка — ограничители как продукт. Bedrock Guardrails — настраиваемые фильтры: вредный контент,
...
для обнаружения prompt injection (внедрения инструкций в текст); в Foundry всё это подаётся как
«Guardrails + controls» — да, Azure переименовал свои контент-фильтры в Guardrails, лишнее очко в пользу
...
В ноябре 2025-го Bedrock перекроил тарифную сетку в четвёрку Reserved / Priority / Standard / Flex, а
старый Provisioned Throughput оставил для моделей постарше и моделей, дообученных под себя.

Есть и третий режим — **пакетный (batch)**: асинхронная обработка для неинтерактивных задач вроде ночной
разметки корпуса или массовой суммаризации. Все три платформы документируют для него цену примерно вдвое
...
И одно объединяет все тарифы: квоты считаются на регион и на модель, и продакшен обязан уметь жить с
ответом 429 на любой платформе. Дисциплина повторных попыток и собственных лимитов из урока про сервинг
никуда не девается, а в [уроке про LLMOps](./llmops.md) мы достроим её маршрутизацией запросов между
моделями и резервными маршрутами (fallbacks).

## Как выбирают на самом деле

Скучная правда: чаще всего платформу определяет уже сделанный облачный выбор. Где лежат твои данные,
где заведены IAM-роли и с кем подписан корпоративный договор — туда и идут за моделями; бенчмарки моделей
решают этот вопрос редко. А вот что действительно различается и заслуживает честного сравнения — четыре
вещи:

1. модели: обслуживает ли платформа те, что тебе нужны, и доступны ли они в твоём регионе;
...
[уроке про LLMOps](./llmops.md) посмотрим на LiteLLM и шлюзы подробнее). Крепче всего держат их
собственные SDK и управляемые уровни: **привязка к поставщику (vendor lock-in)** живёт в батарейках, сам
эндпоинт как раз переносим.
...
- Имена тасуются (две платформы из трёх переименовались примерно за год) — учи категории: каталог
  моделей, приватность и резидентность, управляемый RAG, ограничители, модель оплаты.
...
- Управляемый RAG — скорость в обмен на ручки: умолчание для стандартного корпуса; своё — когда оценка
...
- Тарифы: on-demand с квотами и лимитами, выделенная пропускная способность для ровной нагрузки, пакетный
  режим примерно вдвое дешевле. Ответ 429 планируй на любой платформе.
- Выбирают по облачной прописке, а сравнивают четыре вещи: модели в регионе, соответствие, RAG-уровень,
  экономику ёмкости. Слой приложения держи переносимым — привязка живёт в батарейках.
