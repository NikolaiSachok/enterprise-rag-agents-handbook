...
Слой потока управления и состояния — **LangGraph** и **Microsoft Agent Framework**. Первый строит агента
именно как граф; второй — от Microsoft, целится в корпоративную среду.

Слой мультиагентности — **CrewAI** («экипажи» агентов с ролями) и оркестрации того же **Microsoft Agent
Framework**, унаследованные от AutoGen (агенты, которые *переговариваются* друг с другом). Здесь фреймворк
...
Semantic Kernel и AutoGen — оба теперь в режиме поддержки. Так что запоминай *категории* — под какую
