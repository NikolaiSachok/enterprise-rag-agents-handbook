...
где он снимает с тебя рутину, какую абстракцию навязывает взамен и когда цена этой абстракции того не
стоит. Цель — уметь выбрать фреймворк и пользоваться им осознанно, а не переписывать его заново и не
прятаться за ним.
...
Проще всего понять фреймворк через то, что он *убирает* с твоего стола. Собери агента на голом SDK
...
Начинается всё с самого цикла `рассуждение → действие → наблюдение`: его обвязка, тот самый внешний
...
Если смотреть, к чему сходится большинство фреймворков (чётче всего это видно в LangGraph), — к одной
...
Разложим ландшафт по слоям — не как рейтинг, а как карту философий. Каждый фреймворк тянет в свою сторону,
и полезнее видеть, в какую именно.

Слой интеграций и коннекторов — это **LangChain** и **LlamaIndex**: широкие библиотеки готовых
...
Слой потока управления и состояния — **LangGraph** и **Semantic Kernel**. Первый строит агента именно как
граф; второй — от Microsoft, целится в корпоративную среду и организован вокруг навыков (skills) и
планировщиков.

Слой мультиагентности — **AutoGen** (тоже Microsoft; агенты, которые *переговариваются* друг с другом) и
**CrewAI** («экипажи» агентов с ролями). Здесь фреймворк даёт готовые конструкции под топологии из урока
про команды.

Оговорюсь честно, иначе эта карта соврёт. Границы между слоями размыты: LangChain тоже умеет поток
управления, а фреймворки постоянно копируют друг у друга удачные ходы. И экосистема несётся вперёд —
то, что верно сегодня, устареет к следующему релизу. Так что запоминай не строчку рейтинга и не номер
версии, а *категории*: под какую задачу какой слой. Это переживёт конкретные имена.
...
- Граф с узлами-инструментами и условными рёбрами — базовая форма, из которой собирается почти всё
  остальное.
- Готовый ReAct- или tool-calling-агент — «агент в комплекте», когда тебе не нужен свой граф, а нужен
  рабочий цикл из коробки.
- Память или чекпойнтер, который сохраняет состояние: прогон можно возобновить с места остановки, а разные
...
- Конструкция супервизора или экипажа под мультиагентность — оркестратор из урока про команды, собранный
  за тебя.
- Интеграция трейсинга (например, LangSmith) — выход на наблюдаемость (observability), которую подробно
  разбирает Часть III.
...
это *сквозь слои*, которые писал не ты. Для простого агента голый цикл плюс родной tool-calling провайдера
...
управляемые сложные потоки — чекпоинты, человек-в-цикле, ветвления, мультиагентность — тянись за
граф-фреймворком. Нужен простой агент — бери SDK провайдера напрямую, и не плоди слоёв.
...
них. И ровно поэтому они без швов стыкуются с наблюдаемостью и eval — тем, чем занимается Часть III:
раз агент стал размеченным графом, трейсить и мерять его становится естественным продолжением, а не
отдельной стройкой.

## Что забрать из урока

- **Фреймворк оркестрации** — это готовая обвязка поверх голого цикла: сам цикл `рассуждение → действие →
  наблюдение`, склейка вызова инструментов, состояние и память, поток управления, мультиагентная
...
- Игроки раскладываются по слоям — интеграции (LangChain, LlamaIndex), поток управления и состояние
  (LangGraph, Semantic Kernel), мультиагентность (AutoGen, CrewAI). Границы размыты, экосистема течёт:
  запоминай категории, а не номера версий.
