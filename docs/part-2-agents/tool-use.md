...
параметров — это то, по чему вероятностная модель решает, *когда* и *как* дёрнуть функцию. Размытое описание — и модель вызовет не
...
- **Выдумка поверх результата.** Модель может «дофантазировать» поверх результата — особенно невнятного или
  пустого. Возвращай результат отдельным сообщением, явно помеченным как вывод инструмента; это снижает
...
снимает тот самый случай «разные источники под разные вопросы»: retrieval — в базу знаний, SQL — в таблицы,
веб-поиск — за свежим, калькулятор — за точным счётом. Маршрутизатор (router) из прошлого урока как раз и
