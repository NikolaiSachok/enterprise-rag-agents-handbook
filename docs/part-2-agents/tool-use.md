...
В уроке про agentic RAG ты усвоил ключевой сдвиг: retrieval (поиск) из шага превратился в **действие**,
которое модель выбирает в цикле. Но retrieval — лишь одно из действий. **Использование инструментов (tool use)**,
...
физически не исполняет код. Tool use — это протокол-мост:
...
не касается настоящих систем — она лишь порождает намерение, а исполняет его твой код. Это же
...
- **Результат инструмента (tool result)** — твоя среда выполнения исполняет инструмент и добавляет результат в
...
Вот главная AI-дельта по сравнению с обычным проектированием API. Модель выбирает инструмент и заполняет аргументы,
...
- **Безопасность — новый и серьёзный риск.** Инструмент, который **действует** (пишет, отправляет,
  ...
  читающих и пишущих инструментов, подтверждение на опасные действия. Тогда даже успешная инъекция мало что
сможет сделать.
...
- **Tool use (вызов функций)** — общий механизм: модель вызывает любую внешнюю функцию; retrieval — его
  частный случай.
- Модель **только порождает намерение**, исполняет твой код: она решает «что», твой код делает «как». Это же
